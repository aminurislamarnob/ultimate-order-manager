...
class OrderTableManager {
    ...
    public function uomwoo_add_new_order_admin_list_column( $columns ) {
        ...
        if ( 'yes' === $uomwoo_order_summary ) {
            $columns['uomwoo_order_items'] = esc_html__( 'Order Items', 'ultimate-order-manager-for-woocommerce' );
        }
        ...
        if ( 'yes' === $uomwoo_order_shipping_cost ) {
            $columns['uomwoo_order_shipping_cost'] = esc_html__( 'Shipping Cost', 'ultimate-order-manager-for-woocommerce' );
        }

        //Show Order Shipping Cost
        $uomwoo_order_note = get_option( 'uomwoo_order_note' );
        if ( 'yes' === $uomwoo_order_note ) {
            $columns['uomwoo_customer_order_note'] = esc_html__( 'Order Note', 'ultimate-order-manager-for-woocommerce' );
        }

        //Show Order Payment Method
        $uomwoo_order_pm = get_option( 'uomwoo_order_pm' );
        if ( 'yes' === $uomwoo_order_pm ) {
            $columns['uomwoo_payment_methods'] = esc_html__( 'Payment Method', 'ultimate-order-manager-for-woocommerce' );
        }

        //Show Applied Coupon
        $uomwoo_order_coupon = get_option( 'uomwoo_order_coupon' );
        if ( 'yes' === $uomwoo_order_coupon ) {
            $columns['uomwoo_order_coupon'] = esc_html__( 'Coupon', 'ultimate-order-manager-for-woocommerce' );
        }

        //Show customer IP address
        $uomwoo_customer_ip = get_option( 'uomwoo_customer_ip' );
        if ( 'yes' === $uomwoo_customer_ip ) {
            $columns['uomwoo_customer_ip_address'] = esc_html__( 'IP Address', 'ultimate-order-manager-for-woocommerce' );
        }
        ...
        if ( 'yes' === $uomwoo_customer_os_browser ) {
            $columns['uomwoo_customer_user_agent'] = esc_html__( 'Customer Device, OS, Browser', 'ultimate-order-manager-for-woocommerce' );
        }

        //Show Customer Phone
        $uomwoo_customer_phone = get_option( 'uomwoo_customer_phone' );
        if ( 'yes' === $uomwoo_customer_phone ) {
            $columns['uomwoo_customer_billing_phone'] = esc_html__( 'Phone', 'ultimate-order-manager-for-woocommerce' );
        }

        //Show Customer Email
        $uomwoo_customer_email = get_option( 'uomwoo_customer_email' );
        if ( 'yes' === $uomwoo_customer_email ) {
            $columns['uomwoo_customer_billing_email'] = esc_html__( 'Email', 'ultimate-order-manager-for-woocommerce' );
        }
